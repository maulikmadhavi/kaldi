...
export train_cmd="slurm.pl -p all --time 23:59:00 --mem 500"
...
numLeavesTri1=512   # Number of triphone states
...
#    done < $resdir/model
     ...
     echo $i; 
             ...
             $train_cmd JOB=1:$train_nj logfiles/log_tri1_adapt/$i/align.JOB.log local/tri1_ali.sh JOB $i
             if [ $i -eq 12400 ]; then
                 $train_cmd JOB=1:4 logfiles/log_tri1_adapt/$i/align.JOB.log local/tri1_ali.sh JOB $i
             fi
     done
