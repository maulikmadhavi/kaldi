...
## This code uses the text file to create data/local/dict  data/local/dict and data/local/lm
...
dir=data/local/dict_transfree
lmdir=data/local/nist_lm_transfree
tmpdir=data/local/lm_tmp_transfree
...
# >> $dir/extra_questions.txt || exit 1;
cat $dir/nonsilence_phones.txt | sed ':a;N;$!ba;s/\n/ /g' >> $dir/extra_questions.txt || exit 1;
...
build-lm.sh -i $lmdir/lm_train.text -n 2 \
